...
New sessions are automatically created with git worktree isolation (ticket #21). When the working directory is a git repo and the `useWorktree` setting is enabled (default: true), the bootstrap command is augmented with `-w <session-name>` to leverage Claude Code's built-in worktree support. Each session gets an isolated worktree at `<repo>/.claude/worktrees/<name>`, preventing conflicts between parallel sessions on the same repo. The setting is configurable per-project via the layered preferences model. Session cards display a resolved project name (derived from the git repo root; for Claude worktree paths, the parent repo name is used), the current git branch as a chip badge, and a "Worktree" chip when running in a worktree. Git metadata (branch, worktree status, project name) is cached per session and only refreshed when the working directory changes.

A first-run welcome dialog appears on initial launch (when `firstRunComplete` is false in preferences). It prompts the user to select their terminal app (Terminal.app or iTerm2), explains that git worktree isolation is enabled by default, and points to the settings panel for further configuration. The dialog saves the terminal preference and sets `firstRunComplete: true` on dismissal, so it never appears again. To re-test, set `"firstRunComplete": false` in `~/Library/Application Support/com.muxara.app/preferences.json`.
